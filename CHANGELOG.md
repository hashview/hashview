...
- Sending mail can no longer hang a request forever. Flask-Mail builds its SMTP socket with no timeout, so a relay that accepts a connection and then goes quiet -- a tarpit, a black-holed port 25, a failover mid-session -- parked the thread that was sending, and Hashview sends mail from inside request handlers (stopping a job notifies its owner; a crack upload can fire job-completion mail). There is now a 10-second deadline on the connect and on every subsequent read, tunable with an optional `timeout` key under `[SMTP]` in `config.conf`
