...
### Changed
- Hashview no longer passes `-w 3` (workload profile "High") to hashcat. Upstream retired workload profiles in `083046e7`: `-w` is still accepted but ignored, and every run now gets the launch budget profile 3 used to ask for, so the flag is dead weight in a command the agent runs verbatim. **On a released hashcat (7.1.2 and earlier) profiles still work**, and the default is profile 2, so until the retirement ships in a release this trades some throughput for desktop responsiveness on each agent. Set `HC_EXTRA_ARGS="-w 3"` on an agent to restore the old behaviour; it converges automatically once a release carries the change
