...
# flags as B608 (hardcoded_sql_expressions) at Medium/LOW confidence. All five
# sites are false positives, and nothing operator-supplied reaches the SQL text:
#
#   * `placeholders` expands to ':h0, :h1, ...' / ':l0, :l1, ...' -- SQLAlchemy
#     NAMED BIND PARAMETER names generated by enumerate(), never values. The
#     values travel separately in the `keys` dict and are int()-coerced where
#     they are built (group_hashfiles, merge_group).
#   * `table` and `group_by` come from _CHILD_TABLES below, a module constant of
#     two literal table names and their literal column tuples.
#   * `keeper_id` is bound as :keeper, int()-coerced at the call.
#
# A variable-length IN (...) needs either this or bindparam(expanding=True);
# bandit cannot tell the two apart, so the sites carry `# nosec B608`.
...
def group_hashfiles(conn, hash_ids):
    ...
    rows = conn.execute(text(
        'SELECT f.hash_id, f.hashfile_id, hf.name, f.username'  # nosec B608
...
def merge_group(conn, keeper_id, loser_ids):
    ...
    promoted = conn.execute(text(
        'UPDATE hashes SET cracked = 1,'  # nosec B608
    ...
    for table, redundant_on in _CHILD_TABLES:
        moved = conn.execute(text(
            f'UPDATE {table} SET hash_id = :keeper WHERE hash_id IN ({placeholders})'  # nosec B608
        ), dict(keys, keeper=int(keeper_id)))
    ...
    conn.execute(text(f'DELETE FROM hashes WHERE id IN ({placeholders})'), keys)  # nosec B608
    return stats
