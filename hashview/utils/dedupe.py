...
def group_hashfiles(conn, hash_ids):
    ...
    if not hash_ids:
        return {}
    keys = {f'h{i}': int(v) for i, v in enumerate(hash_ids)}
    ...
    rows = conn.execute(text(
        ...
        f' WHERE f.hash_id IN ({placeholders})'
...
def merge_group(conn, keeper_id, loser_ids):
    ...
    if not loser_ids:
        return {'keeper': int(keeper_id), 'deleted': [], 'promoted_crack': False,
                'relinked': {}, 'collapsed': {}}
    keys = {f'l{i}': v for i, v in enumerate(loser_ids)}
    placeholders = ', '.join(':' + k for k in keys)
    ...
    promoted = conn.execute(text(
        ...
        f'               WHERE l.id IN ({placeholders}) AND l.cracked = 1'
        ...
        f'   AND EXISTS (SELECT 1 FROM (SELECT * FROM hashes) x WHERE x.id IN ({placeholders})'
        '                 AND x.cracked = 1)'
    ), dict(keys, keeper=int(keeper_id)))
    stats['promoted_crack'] = bool(promoted.rowcount)

    for table, redundant_on in _CHILD_TABLES:
        moved = conn.execute(text(
            f'UPDATE {table} SET hash_id = :keeper WHERE hash_id IN ({placeholders})'
        ), dict(keys, keeper=int(keeper_id)))
        ...
        collapsed = conn.execute(text(
            f'DELETE FROM {table} WHERE hash_id = :keeper AND id NOT IN ('
    ...
    conn.execute(text(f'DELETE FROM hashes WHERE id IN ({placeholders})'), keys)
    return stats
