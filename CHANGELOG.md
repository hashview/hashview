...
- Incomplete jobs now have the Info and Analytics buttons every other status already had on the jobs list. They were hidden for exactly the status an operator most needs to look into -- a job that either never finished being built, or ran and ended with work outstanding. The info panel was being rendered for those jobs all along; only the button to open it was missing. Analytics is offered whenever the job has a hashfile rather than based on its status, so a job abandoned at the first step of the creation wizard shows the control disabled instead of linking to every hashfile the customer owns
