...
- Entries whose file is gone **and that no task uses** are now cleaned up on their own. The hourly sweep removes them -- but never on the sweep that first notices one, only on a later sweep, so the alert naming the entry always comes first -- and sends a separate "orphaned catalog entries removed" notification listing exactly what went. An entry a task still references is never touched: that one needs a human to choose between restoring and deleting, and nothing is removed at all while a whole control directory looks empty, which is a lost mount rather than a lost file. Such an entry previously had no exit, since restoring needs a file that may no longer exist anywhere and deleting meant hunting row by row; it just sat in the catalog inflating the "missing on disk" count. The Rules and Wordlists pages badge these entries `UNUSED` and say what will happen to them, and `GET /v1/rules` / `GET /v1/wordlists` carry an `orphaned` flag per row. Turn it off under Settings -> General -> "Clean up stranded catalog entries" (#494)
- **Manual prune buttons**: Administrators can remove stranded catalog entries directly from Settings -> Data management via **Prune stranded wordlists** and **Prune stranded rules** buttons. The manual path deliberately omits two safeguards of the automatic hourly `CATALOG_HEALTH` sweep: it does not wait for the `file_missing_notified` latch -- the sweep enforces this to avoid destroying rows before an operator is alerted, but a human clicking the button is that review -- and it does not require the `catalog_prune_orphans` toggle to be enabled -- the toggle arms the sweep, but if applied to the manual path would eliminate cleanup entirely when disabled, contradicting the alert that tells operators to disable it "if you would rather remove them by hand". The manual path keeps the other two guardrails: never removing an entry a task references, and never pruning when a whole control directory has no healthy files. Removals are audited as `rule.pruned` / `wordlist.pruned` with `manual=1` and the performing admin as actor (#502)
...
### Changed
- **Times are now shown in your own timezone.** Every timestamp Hashview records -- job starts and ends, uploads, agent check-ins, recovered passwords, the audit log and the application log -- is stored in UTC, and the pages convert to whatever timezone your browser is set to. Previously a timestamp was written in the timezone of whichever machine happened to write it: the web server for job times, the database server for agent check-ins, and UTC for logins. On a single-machine install running UTC those all agreed, which is why this was invisible; anywhere else they did not, and a job could appear to have finished before it started. Hovering a time shows the full date with your timezone named. If your install is not on UTC, times recorded before this release were written in the old local zone and will read shifted by that offset -- new ones are correct
- The three light themes (Paper / Invert / Clean) were repainted from the design system, and the top bar now follows the theme instead of staying near-black on all of them. The accent colours -- the ones that print status pills, ETAs, hash types and agent temperatures -- were the mid-tone hues borrowed from the dark theme, which on a pale background left most of them below the accessibility floor for readable text: amber on Paper measured 2.3:1 where 4.5:1 is the minimum. Each is now the darker ink shade, and the faded text tone is no longer semi-transparent (it was blending into the page and all but disappearing). Nineteen colours across the three themes were under that floor; none are now. **The dark theme is unchanged.**
- **The per-job runtime cap now measures time a job was actually being worked on, not time since it started.** `Settings.max_runtime_jobs` counted wall-clock from the moment a job began, which charged it for every hour it spent waiting for an agent -- and waiting is the normal consequence of priorities. A job at priority 3 loses the fleet to a job at priority 5, sits untouched for hours, and is then killed for a runtime it never had. A job now accrues against its cap only while at least one of its tasks is running on an agent, so time nobody spent working on it costs it nothing. The trade-off is deliberate and worth knowing: the setting no longer bounds how long a job can sit in the system, only how much of the fleet's time it may consume, so a repeatedly starved job can stay in the queue far longer in wall-clock terms than the cap suggests. Jobs already running when you upgrade start this clock at zero and get a full allowance from that point; nothing is expired by the upgrade itself. The running-jobs dashboard's *elapsed* and *left* readouts now count this same worked-time clock rather than wall-clock, so a starved job no longer shows its time-left ticking down to zero while it keeps running
...
- The unit suite passes in any timezone. Thirty-four test seeds wrote `datetime.now()` (naive local) into columns the app reads as naive UTC, so the suite only ever agreed with the code it tests on a machine set to UTC — invisible in CI, which runs UTC, and broken on contributors' machines. Swept across UTC-11 to UTC+14: 15 tests failed before, none do now, and a source-level guard keeps the local clock out of `tests/` the way one already keeps it out of `hashview/` (#522)
- The hashcat interop contract now distinguishes a flag hashcat *documents* from one it still *accepts*. It previously checked only the text of `--help`, which is a proxy for what actually matters, and the proxy broke in both directions: hashcat retired workload profiles and dropped `-w` from `--help` while still honouring it, which read as an interop break when nothing had broken, while a flag quietly becoming a hard error — which would kill every job at launch — would not have been caught at all. The live tiers now probe each flag against a real binary
- A fourth hashcat interoperability tier builds hashcat from git master each week and re-runs the same contracts against it, including a re-derivation of the hash-case table. The other three tiers only ever see a release, so the earliest they can report a break is the day one ships. This one never gates anything and comments on a single tracking issue instead of failing, because master is not a release — a red mark there is advance warning, not a defect
- The hashcat interoperability matrices no longer test anything below 7.0. hashcat 6.2.6 was dropped from both the version matrix and the Kerberos interop matrix, along with its captured fixtures, leaving 7.0.0 through 7.1.2. Nothing in Hashview gates on a hashcat version, so a 6.x binary will still run — it is simply no longer verified by CI
...
### Fixed
- A hash pasted in upper case is now stored in the case hashcat reports it, so the crack finds its way back to the right row. hashcat accepts hex in either case and always reports it in lower case; Hashview matched a recovered hash to its row by comparing the two exactly, so an upper-case hash -- which is how SQL Server Management Studio, Oracle and plenty of export tools hand them over -- was cracked by an agent and then silently failed to match. The row stayed marked uncracked for ever, its password never reached the `(DYNAMIC) All Recovered Passwords` wordlist, and every later job attacked it again. Four hash types were normalised before this (MySQL 4.1/5, NTLM, MSSQL 2012/2014 and DCC2); 292 are now, including MD5, SHA1, SHA2-256, and MSSQL 2000 and 2005 -- the pair the report came in on, and the reason MSSQL 2012/2014 appeared to work while its two siblings did not. This is deliberately not a blanket lower-casing, which would have broken hashes that are correct today: nine types report in UPPER case (Oracle 7 and 12, the SAP CODVN family, RACF), fourteen report the line back exactly as it was given to them, and a type with a text salt has that salt echoed untouched, so folding it would change the hash. Every rule was derived from hashcat itself and kept only where it reproduced hashcat's own output for every spelling of the hash; a type where it did not is left exactly as it was. **Hashes already imported in the wrong case are not repaired by this change** -- re-upload the hashfile to correct them
- A `user:hash` file uploaded as MySQL 4.1/5 or MSSQL 2012/2014 no longer stores the username as part of the hash. Those two types took a branch that lower-cased the whole line and stored *that* as the hash, so the row read `alice:fcf7c1b8...` where every other type stored `fcf7c1b8...`. Such a hash can never match a crack whatever its case, is counted as a distinct hash from the same one uploaded without a username, and is dispatched to agents in a form hashcat will not load
- Uploading a hashfile as the wrong MSSQL version is now caught at upload instead of failing silently much later. MSSQL 2000, 2005 and 2012/2014 (hashcat modes 131, 132 and 1731) had no validation at all, so picking the wrong one of the three -- easy, since they are the same product and two of them share a prefix -- was accepted without comment. The job was created, queued and dispatched, and only then did hashcat refuse to load the file, leaving an error in an agent log rather than on the form where the mistake was made. All three are now checked, along with Sybase ASE (8000), which had the same gap for the same reason. The rules were confirmed against hashcat itself: they accept exactly the files it accepts
- An agent-health check that could not read the database's clock no longer declares the entire fleet offline. The check compared agent check-in times against a cutoff taken from the database, and if that one query failed it silently fell back to a different clock -- so on a database not set to UTC every agent looked hours stale at once, and admins were paged about the whole fleet because of a single failed query, at exactly the moment the database was already struggling. With one clock there is nothing to fall back to and the fallback is gone
- Uploading a hashfile that ends in blank lines no longer fails with a server error. Most editors leave a trailing newline and an export concatenated from two files often has a few, so this was easy to hit: the upload validated fine -- validation skips blank lines on purpose -- and then the import tripped over the very lines validation had waved through, returning an error 500 from the job wizard with nothing to explain it. Affected pwdump, shadow, NetNTLM and Kerberos files. A `$hash`-only file behaved worse than an error: it imported silently and left a row with an empty hash in the database, which then counted toward totals and duplicate detection
- Hashes uploaded through `POST /v1/hashfiles/upload/...` are now normalised the same way the web upload normalises them. The importer decides how to normalise a hash by comparing the hash type against text, but the API route hands it over as a number, so every one of those decisions was silently skipped. The visible consequence was NTLM: hashcat reports hashes in lower case, the API stored them exactly as uploaded, and the two could never match -- so a hash that had genuinely been cracked stayed marked uncracked forever, its password never reached the `(DYNAMIC) All Recovered Passwords` wordlist, and the same hash was re-attacked by every later job. DCC2 and Kerberos AS-REP uploads also lost their usernames (AS-REP rows were given the whole ciphertext as the username). Hashfiles uploaded through the web UI were never affected. **Hashes already imported through the API keep the form they were stored in and are not repaired by this change** -- re-upload the hashfile to correct them
...
- An agent configured with `HC_EXTRA_ARGS = --hwmon-temp-abort=N` (or any other flag hashcat refuses in benchmark mode) could never benchmark at all: `hashcat -b` exits immediately with "Can't change --hwmon-temp-abort in benchmark mode." and no `Speed.#` line, so the agent reported every hash type unsupported and the server's benchmark-first gate kept re-asking it forever -- an agent stuck permanently idle with jobs stuck permanently queued, and nothing in either the server or agent logs pointing at why. Benchmark-incompatible flags are now stripped from `HC_EXTRA_ARGS` before the benchmark run only; the real cracking run still gets the full flag set
...
- A job that outlived its runtime cap while no agent was working on it is now stopped within a minute, instead of hanging on the dashboard until something happened to look at it. The cap was only ever evaluated inside an agent's check-in, against the one job that agent was about to be given work for -- so a job whose agents had all moved on to higher-priority work was never re-examined, and kept its Running status, its place on the dashboard and its row in the queue until the other job finished hours later and the next agent to check in tripped the check on its way past. A job that outlived its cap *and* lost its last agent was never re-examined at all. A sweep now checks every live job once a minute, whether or not anybody is asking
...
- Every GitHub Actions step is pinned to a commit SHA rather than a mutable major tag, and the database images CI tests against are pinned by digest. A tag such as `actions/checkout@v5` can be repointed by its owner at any time, and the action then runs in a job holding a checkout of this repo and a `GITHUB_TOKEN`. No version was upgraded — each pin is the commit its existing tag already resolved to — and a test fails the build if a mutable ref comes back. Deployment images (`Dockerfile`, `Dockerfile.agent`, `docker-compose.yml`) are deliberately left on floating tags so upstream security patches still reach a deployment without a repo change
