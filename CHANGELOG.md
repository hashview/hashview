...
- Every CI workflow ran twice on each commit pushed to a branch with a pull request open. `push` and `pull_request` are separate events, and eight workflows subscribed to both with no branch filter on `push`, so a single commit spent two full unit matrices, two Playwright runs, two end-to-end compose stacks and two database-parity matrices. It also broke the check rollup: when one of a pair failed on a transient such as a registry 502 during an image build, re-running the other did not clear it and the pull request stayed red against a commit whose tests had passed. `push` is now scoped to `main` and the release dev branch, so a pull-request branch is covered once by `pull_request` and a merge is covered once by `push`, and a test fails the build if the duplicating shape returns. Pushing to a branch with no pull request open no longer starts CI; open the pull request to get it
...
- The bandit baseline no longer hides regressions. Bandit matches a finding to a baseline entry on file, test ID, and message text while ignoring the line number, so an entry whose code was fixed did not go inert — it stayed a standing file-scoped exemption for that test ID. Seven of the fourteen committed entries had stopped reproducing, including the `shell=True` removed from the agent, which meant reintroducing `shell=True` anywhere in that file would have passed the gate silently. The baseline is regenerated, and a new CI step fails the build whenever it holds findings that no longer reproduce, so it cannot drift again (#426)
