...
- Entries whose file is gone **and that no task uses** are now cleaned up on their own. The hourly sweep removes them — but never on the sweep that first notices one, only on a later sweep, so the alert naming the entry always comes first — and sends a separate "orphaned catalog entries removed" notification listing exactly what went. An entry a task still references is never touched: that one needs a human to choose between restoring and deleting. Such an entry previously had no exit at all, since restoring needs a file that may no longer exist anywhere and deleting meant hunting row by row; it just sat in the catalog inflating the "missing on disk" count. The Rules and Wordlists pages badge these entries `UNUSED` and say what will happen to them, and `GET /v1/rules` / `GET /v1/wordlists` carry an `orphaned` flag per row. Turn it off under Settings → General → "Clean up stranded catalog entries" (#494)
