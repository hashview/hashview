...
def client(base_url, seeded):
    ...
    response = session.post(f"{base_url}/login", data=payload, timeout=10)
    assert "/login" not in response.url, "login failed for the seeded test user"
    return session


def _analytics(client, base_url, **params):
    response = client.get(f"{base_url}/analytics", params=params, timeout=60)
...
def test_cracked_total_counts_each_account_once(client, base_url, seeded):
    """The 'of N cracked' corpus size must not double-count an account whose
    hash appears in two hashfiles."""
    html = _analytics(client, base_url, **_scope_a(seeded))
    total = _number(r">of ([\d,]+) cracked<", html, "the cracked corpus total")
    assert total == EXPECTED_CRACKED_ACCOUNTS


def test_shared_password_card_counts_distinct_accounts(client, base_url, seeded):
...
def test_username_equals_password_card_counts_distinct_accounts(client, base_url, seeded):
...
def test_shared_download_finds_the_plaintext_the_card_displays(client, base_url, seeded):
    ...
    response = client.post(
        f"{base_url}/analytics/download/shared",
...
def test_blank_password_download_returns_its_group(client, base_url, seeded):
...
def test_shared_zip_contains_one_file_per_group_on_the_card(client, base_url, seeded):
    """The zip is the 'download them all' escape hatch for the capped card, so
    it must hold exactly the groups the card counted."""
    response = client.get(
        f"{base_url}/analytics/download/shared_zip",
...
def test_fig9_download_lists_the_accounts_that_share_a_password(client, base_url, seeded):
    ...
    response = client.get(
        f"{base_url}/analytics/download/fig9", params=_scope_a(seeded), timeout=60
    )
    assert response.status_code == 200
...
def test_fig8_download_matches_the_username_equals_password_card(client, base_url, seeded):
    """The username=password download must contain every account the card counts,
    including the case-differing one."""
    response = client.get(
        f"{base_url}/analytics/download/fig8", params=_scope_a(seeded), timeout=60
    )
    assert response.status_code == 200
...
def test_non_numeric_scope_ids_are_rejected_not_crashed(client, base_url):
    """The download routes 400 on a non-numeric id; the page itself must not
    500 on the same input."""
    response = client.get(
        f"{base_url}/analytics", params={"customer_id": "abc"}, timeout=60
    )
    assert response.status_code < 500, "non-numeric customer_id crashed /analytics"


def test_hostile_plaintext_and_username_are_escaped(client, base_url, seeded):
...
def test_shared_card_caps_rendered_rows_but_reports_the_true_total(client, base_url, seeded):
    """Regression for the 50MB analytics response: the card renders at most
    PREVIEW_LIMIT rows while still reporting every group."""
    html = _analytics(client, base_url, customer_id=seeded["customer_b"])
    groups = _number(r">(\d+) groups<", html, "the shared-password group count")
