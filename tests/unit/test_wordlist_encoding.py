...
def _regen_download_from_stored(monkeypatch):
    """The /v1 dynamic download regenerates into a per-request temp file instead
    of compressing the stored .txt. For these encoding round-trip tests, make
    ...
    monkeypatch.setattr(routes_mod, "update_dynamic_wordlist", _fake)
